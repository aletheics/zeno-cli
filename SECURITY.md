...
Zeno and also where the boundaries are.

In general Zeno is a coding agent that runs locally within the security boundary
...
Zeno treats the local user account and files writable by that account as inside
the same trust boundary as the Zeno process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Zeno configuration, they can generally influence Zeno or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Zeno grants that write access or crosses
an operating-system privilege boundary.

Zeno relies on users installing trustworthy extensions and loading trustworthy
skills and only to use zeno within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in zeno or another package in
...
- Local code execution or sandboxing behavior (the Zeno coding agent intentionally does not have a sandbox)
- Behavior of zeno extensions or skills installed by the user
...
- Public internet exposure of a Zeno installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.zeno`,
  `~/.zeno/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Zeno itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the zeno coding agent.
...
For example, a report showing that malicious contents written to a trusted Zeno
configuration file cause Zeno to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Zeno.  For exposed-secret reports,
