...
await session.readList(workspaceEvents("zeno"), { limit: 100 }, context);
```
...
`AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` come from `@aletheics/zeno-ai`. Do not define a second frame codec or reducer.
