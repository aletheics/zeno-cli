...
Durable agent harness for zeno: one session file, any number of conversations, every piece of work
...
```typescript
import type { Call } from '@aletheics/zeno-agent';
import { BACKGROUND_CONTEXT, withCancel } from '@aletheics/chord/context';
...
```bash
npm install @aletheics/zeno-agent
```

## Quick Start

```typescript
import { Harness, JsonlStorage, systemSections, type Call } from '@aletheics/zeno-agent';
import { BACKGROUND_CONTEXT } from '@aletheics/chord/context';
import { readTool, writeTool, bashTool } from '@aletheics/zeno-agent/tools';
import { generationKind } from '@aletheics/zeno-agent/kinds';
...
entries contain the exact rendered pi-ai messages, including complete tool additions/removals. Zeno-ai
...
```typescript
import { Type, type Tool } from '@aletheics/zeno-agent';
