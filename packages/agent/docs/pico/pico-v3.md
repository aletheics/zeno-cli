...
not proof that a provider received them. Zeno-ai owns provider translation and best-effort cache
...
```ts
import { AssistantMessageFrameEncoder, type AssistantMessageFrame } from "@aletheics/zeno-ai";
...
import { createContextKey, withAbortSignal, withContextValue } from "@aletheics/chord/context";
...
**Zeno-ai owns provider translation.** Pico passes only `{ messages }`, with neither top-level
