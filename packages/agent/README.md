# @aletheics/zeno-agent-core

Stateful agent with tool execution and event streaming. Built on `@aletheics/zeno-ai`.

## Installation

```bash
npm install @aletheics/zeno-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@aletheics/zeno-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
Transport-neutral facet-service primitives live in `@aletheics/chord`. The agent core does not export the service runtime.
...
```typescript
declare module "@aletheics/zeno-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@aletheics/zeno-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@aletheics/zeno-agent-core";
