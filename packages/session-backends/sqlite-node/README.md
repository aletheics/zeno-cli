# @aletheics/zeno-session-backend-sqlite-node

Node `node:sqlite` Session backend for `@aletheics/zeno-agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@aletheics/zeno-agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@aletheics/zeno-session-backend-sqlite-node";
