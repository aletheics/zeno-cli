import type { ByteTransport, ByteTransportFactory, ByteTransportHandlers, Client } from "@aletheics/zeno-client";
import { DEFAULT_MAX_FRAME_LENGTH, type ServerId } from "@aletheics/zeno-protocol";
import type { Server } from "@aletheics/zeno-server";
...
export class RadiusRelayHost {
	...
	async close(): Promise<void> {
		...
		this.#writer?.close();
		this.#socket?.close(1000, "Zeno server stopped");
...
class RadiusClientByteTransport implements ByteTransport {
	...
	close(): void {
		...
		this.#socket.close(1000, "Zeno client closed");
	}
