...
import type { Context } from "@aletheics/chord";
import type { FacetBundleArtifact } from "@aletheics/chord/node";
import {
	BACKGROUND_CONTEXT,
...
import { NodeExecutionEnv } from "@aletheics/zeno-agent-core/node";
import { Client, ServerError as ClientServerError, DisconnectedError } from "@aletheics/zeno-client";
import { createUnixTransportFactory, type UnixServerRoute } from "@aletheics/zeno-client/unix";
import { isServerId, type ServerId } from "@aletheics/zeno-protocol";
import {
...
} from "@aletheics/zeno-server";
import { createUnixServer, getUnixSocketPath } from "@aletheics/zeno-server/unix";
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to PI_SERVER_DIR or ~/.zeno/server. */
	readonly directory?: string;
