...
import { DEFAULT_RADIUS_GATEWAY } from "@aletheics/zeno-ai/providers/radius-config";
import { type Container, type EditorComponent, hyperlink, type TUI } from "@aletheics/zeno-tui";
...
async function tryShareViaRadius(tmpFile: string, context: SessionShareContext): Promise<boolean> {
	...
	try {
		...
		url.searchParams.set("title", "Zeno session");
