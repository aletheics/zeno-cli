...
Extensions can register either a complete pi-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Zeno composes `models.json` overrides above registered native providers.
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. zeno waits for the factory before startup continues, so the provider is available during interactive startup and to `zeno --list-models`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map zeno levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@aletheics/zeno-ai";
...
Credentials are persisted in `~/.zeno/agent/auth.json`:
...
When a request exceeds the model's context window, zeno can recover automatically by compacting the conversation and retrying. This recovery only kicks in if zeno recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of zeno's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/pi/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message zeno does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase zeno recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before zeno tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what zeno checks. With this in place, zeno will:
...
- Match a provider-specific pattern, not zeno's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of zeno's normal retry-with-backoff path.
...
interface ProviderModelConfig {
  ...
  /** Maps zeno thinking levels to provider/model-specific values; null marks a level unsupported. */
