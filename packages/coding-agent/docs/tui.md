> zeno can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@aletheics/zeno-tui`](https://github.com/earendil-works/pi/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@aletheics/zeno-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer's `showHardwareCursor` constructor argument or `setShowHardwareCursor(true)`. Zeno also maps `PI_HARDWARE_CURSOR=1` to this setting before it creates its renderer. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@aletheics/zeno-tui";
...
Import from `@aletheics/zeno-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@aletheics/zeno-tui";
```
...
```typescript
import { matchesKey, Key } from "@aletheics/zeno-tui";
...
```typescript
import { MouseRegion } from "@aletheics/zeno-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@aletheics/zeno-tui";
...
```typescript
import { getMarkdownTheme } from "@aletheics/zeno-coding-agent";
import { Markdown } from "@aletheics/zeno-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@aletheics/zeno-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@aletheics/zeno-coding-agent";
import { DynamicBorder } from "@aletheics/zeno-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@aletheics/zeno-tui";
...
```typescript
import { BorderedLoader } from "@aletheics/zeno-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@aletheics/zeno-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@aletheics/zeno-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@aletheics/zeno-tui";
...
Customize the inline working indicator shown while zeno is streaming a response.
...
// Restore zeno's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@aletheics/zeno-tui";
