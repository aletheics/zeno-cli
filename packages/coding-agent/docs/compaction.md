...
LLMs have limited context windows. When conversations grow too long, Zeno uses compaction to summarize older content while preserving recent work. This page covers both auto-compaction and branch summarization.

**Source files** ([zeno](https://github.com/earendil-works/pi)):
...
For TypeScript definitions in your project, inspect `node_modules/@aletheics/zeno-coding-agent/dist/`.

## Overview

Zeno has two summarization mechanisms:
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.zeno/agent/settings.json` or `<project-dir>/.zeno/settings.json`). This leaves room for the LLM's response.

During a multi-turn agent run, Zeno checks this threshold after tools finish and their results are appended, before starting the next assistant response. If the threshold is crossed, Zeno compacts inside the same agent run and resumes with the summary and retained messages. It skips this between-turn check when the completed tool batch terminates the run and no queued message requires another response. Zeno also checks the threshold before a new user prompt and after a low-level agent run ends.

You can also trigger manually with `/compact [instructions]`, where optional instructions focus the summary.

### How It Works

1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.zeno/agent/settings.json` or `<project-dir>/.zeno/settings.json`) is reached
...
On repeated compactions, the summarized span starts at the previous compaction's kept boundary (`firstKeptEntryId`), not at the compaction entry itself, falling back to the entry after the previous compaction if that kept entry cannot be found in the path. This preserves messages that survived the earlier compaction by including them in the next summarization pass as well. Zeno also recalculates `tokensBefore` from the rebuilt session context before writing the new `CompactionEntry`, so the token count reflects the actual pre-compaction context being replaced.
...
For split turns, Zeno generates two summaries and merges them:
...
When you use `/tree` to navigate to a different branch, Zeno offers to summarize the work you're leaving. This injects context from the left branch into the new branch.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, zeno extracts file operations from:
...
```typescript
import { convertToLlm, serializeConversation } from "@aletheics/zeno-coding-agent";
...
Configure compaction in `~/.zeno/agent/settings.json` or `<project-dir>/.zeno/settings.json`:
