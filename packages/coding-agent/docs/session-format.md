...
```
~/.zeno/agent/sessions/--<path>--/<timestamp>_<session-id>.jsonl
```

By default, `<session-id>` is a UUID. Callers can supply a custom ID through the SDK or `--session-id`. For `<path>`, Zeno removes the leading path separator and replaces `/`, `\\`, and `:` with `-`.

## Deleting Sessions

Sessions can be removed by deleting their `.jsonl` files under `~/.zeno/agent/sessions/`.

Zeno also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, zeno uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([zeno](https://github.com/earendil-works/pi)):
...
For TypeScript definitions in your project, inspect `node_modules/@aletheics/zeno-coding-agent/dist/` and `node_modules/@aletheics/zeno-ai/dist/`.
...
`firstKeptEntryId` is required. It identifies the first entry retained from before the compaction entry. When rebuilding context, Zeno replaces older summarized entries with the compaction summary and keeps the range beginning at this entry.
