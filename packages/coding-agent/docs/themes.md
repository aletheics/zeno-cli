> zeno can create themes. Ask it to build one for your setup.
...
Zeno loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.zeno/agent/themes/*.json`
- Project: `.zeno/themes/*.json` (only after the project is trusted)
...
On first run, zeno detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.zeno/agent/themes
vim ~/.zeno/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, zeno reloads it automatically for immediate visual feedback.
...
Zeno uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, zeno falls back to the nearest approximation.
