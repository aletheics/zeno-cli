> zeno can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to zeno's agent capabilities. Use it to embed zeno in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @aletheics/zeno-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@aletheics/zeno-coding-agent";
...
The `Agent` class (from `@aletheics/zeno-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.zeno/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.zeno/extensions/`)
- Project skills:
  - `.zeno/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.zeno/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.zeno/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@aletheics/zeno-ai";
import { ModelRuntime } from "@aletheics/zeno-coding-agent";
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.zeno/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `PI_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@aletheics/zeno-ai";
import { createAgentSession, ModelRuntime } from "@aletheics/zeno-coding-agent";

// Default: uses ~/.zeno/agent/auth.json and ~/.zeno/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@aletheics/zeno-coding-agent";
...
The `edit` tool returns `details.diff` for Zeno's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@aletheics/zeno-coding-agent";
...
import { createAgentSession, defineTool } from "@aletheics/zeno-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.zeno/agent/extensions/`, `.zeno/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@aletheics/zeno-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@aletheics/zeno-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@aletheics/zeno-coding-agent";
...
1. Global: `~/.zeno/agent/settings.json`
2. Project: `<cwd>/.zeno/settings.json`
