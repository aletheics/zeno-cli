...
```bash
git clone https://github.com/aletheics/zeno-cli
cd zeno-cli
npm install
npm run build
```
...
```bash
/path/to/zeno-cli/zeno-test.sh
```

The script can be run from any directory. Zeno keeps the caller's current working directory.
...
```bash
PI_EXPERIMENTAL=1 ./zeno-test.sh server
PI_EXPERIMENTAL=1 ./zeno-test.sh client
```

`PI_SERVER_DIR` overrides the server profile and socket directory (default: `~/.zeno/server`). `PI_SERVER_ID` selects the logical server ID when `--server-id` is omitted.

The `client` and `experimental/plugin` package subpaths resolve only under the `source` condition in a checkout. Their implementations and the server/client commands are excluded from npm packages and standalone binaries. `zeno-client`, `zeno-protocol`, and `zeno-server` are development dependencies of coding-agent, not runtime dependencies. The local SDK and stdio RPC API are unchanged.
...
`/debug` (hidden) writes to `~/.zeno/agent/zeno-debug.log`:
