...
import { BACKGROUND_CONTEXT } from "@aletheics/chord/context";
import { readFacetBundleManifest } from "@aletheics/chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			...
			`${JSON.stringify({
				...
				version: "1.0.0",
				peerDependencies: {
					"@aletheics/chord": "^0.84.4",
					"@aletheics/zeno-coding-agent": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@aletheics/chord"; import { SlashCommands } from "@aletheics/zeno-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@aletheics/chord"; import { SlashCommands } from "@aletheics/zeno-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					...
					version: "1.0.0",
					peerDependencies: { "@aletheics/chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "tui.ts"),
				'import { defineFacet } from "@aletheics/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@aletheics/chord"; import { SlashCommands } from "@aletheics/zeno-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		...
		expect(manifest.plugin).toEqual({ id: "@aletheics/zeno-example-plugin", version: "1.0.0" });
		...
		expect(loaded.facets.map(({ id }) => id)).toEqual(["@aletheics/zeno-example-plugin/tui"]);
