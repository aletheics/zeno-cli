...
import { type Context, createFacetHost, defineFacet, defineService } from "@aletheics/chord";
import { BACKGROUND_CONTEXT } from "@aletheics/chord/context";
import { Client, ServerError as ClientServerError } from "@aletheics/zeno-client";
import { createUnixTransportFactory } from "@aletheics/zeno-client/unix";
...
beforeEach(async () => {
	...
	vi.stubEnv("ZENO_CODING_AGENT_DIR", agentDir);
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			...
			writeFile(
				...
				'import { defineFacet, defineService } from "@aletheics/chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
