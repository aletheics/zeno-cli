...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".zeno", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".zeno", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".zeno", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".zeno"), { recursive: true });
			writeFileSync(join(cwd, ".zeno", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".zeno"), { recursive: true, force: true });
