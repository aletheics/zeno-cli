...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@aletheics/zeno-tui";
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			...
			cwd,
			contextFiles: [
				{ path: path.join(home, ".zeno", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.zeno/agent/AGENTS.md, AGENTS.md");
	...
	test("shows system prompt context paths before project context files", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			systemPromptSource: { path: path.join(cwd, ".zeno", "SYSTEM.md") },
			appendSystemPromptSources: [{ path: path.join(cwd, ".zeno", "APPEND_SYSTEM.md") }],
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain(".zeno/SYSTEM.md, .zeno/APPEND_SYSTEM.md, AGENTS.md");
	});
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.zeno/agent/AGENTS.md");
		expect(output).toContain("~/Development/pi-mono/AGENTS.md");
		expect(output).not.toContain("~/.zeno/agent/AGENTS.md, AGENTS.md");
	});
