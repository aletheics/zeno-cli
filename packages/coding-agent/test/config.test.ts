...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@aletheics");
	const packageDir = join(scopeDir, "zeno-coding-agent");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@aletheics/zeno-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @aletheics/zeno-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@aletheics/zeno-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@aletheics/zeno-coding-agent")).toBe(
			"Update @aletheics/zeno-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@aletheics/zeno-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@aletheics/zeno-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @aletheics/zeno-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@aletheics/zeno-coding-agent", undefined, {
			packageName: "@aletheics/zeno-coding-agent",
			installSpec: "@aletheics/zeno-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@aletheics/zeno-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @aletheics/zeno-coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@aletheics/zeno-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @aletheics/zeno-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@aletheics/zeno-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @aletheics/zeno-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@aletheics/zeno-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @aletheics/zeno-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@aletheics/zeno-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@aletheics", "zeno-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@aletheics/zeno-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@aletheics/zeno-coding-agent")).toContain(
