...
the Zeno monorepo, but it is not a Zeno package: it does not depend on any other Zeno
...
`@aletheics/chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@aletheics/chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@aletheics/chord/delta`:

```ts
import { apply, track } from "@aletheics/chord/delta";
...
`@aletheics/chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { bundleFacetPackage } from "@aletheics/chord/bundler";
...
```ts
import { createFacetBundleLoader } from "@aletheics/chord/node";
