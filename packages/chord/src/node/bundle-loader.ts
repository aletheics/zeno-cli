...
function resolveExternalTarget(specifier: string, resolver?: FacetBundleExternalResolver): string {
	...
	if (specifier === "@aletheics/chord") return new URL(`../index.${extension}`, import.meta.url).href;
	if (specifier === "@aletheics/chord/context") {
		return new URL(`../context/index.${extension}`, import.meta.url).href;
	}
	if (specifier === "@aletheics/chord/node") return new URL(`../node.${extension}`, import.meta.url).href;
