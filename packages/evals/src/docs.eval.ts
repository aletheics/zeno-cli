...
import { Type } from "@aletheics/zeno-ai";
...
describeEval("Coding agent documentation", { harness: documentationAuditHarness }, (it) => {
	it.for(documentationPages)("$path matches the implementation", { timeout: 300_000 }, async ({ path }, { run }) => {
		...
		const result = await run(`Audit one Zeno documentation page against the repository implementation.

Documentation page: ${documentationPath}
Repository root: ${repositoryRoot}

Read the complete documentation page. Identify its concrete claims about Zeno behavior, public APIs, configuration, commands, formats, defaults, and supported values.
