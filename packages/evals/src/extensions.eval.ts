...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasZenoDocs: boolean;
...
function createExtensionAuthoringHarness(name: string, transformSystemPrompt?: (defaultPrompt: string) => string) {
	return createPiCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			const extensionPath = join(session.sessionManager.getCwd(), ".zeno", "extensions", "hello.ts");
			...
			return {
				...
				systemPromptHasZenoDocs: session.systemPrompt.includes("\nZeno documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Zeno system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Zeno system prompt has no working-directory section.");
...
const ExtensionAuthoringJudge = createJudge<PiCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@aletheics/zeno-coding-agent")) {
				failures.push("extension does not import the canonical @aletheics/zeno-coding-agent package");
			}
	...
	describeEval(
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				...
				expect(result.output.systemPromptHasZenoDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
