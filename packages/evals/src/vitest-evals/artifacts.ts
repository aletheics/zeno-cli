...
interface SourceArtifact extends TestArtifactBase {
	type: "@aletheics/zeno-evals:source";
	runId: string;
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@aletheics/zeno-evals:session" && artifact.type !== "@aletheics/zeno-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@aletheics/zeno-evals:session" ? "sessions" : "sources";
