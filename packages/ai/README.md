...
```bash
npm install @aletheics/zeno-ai
```

TypeBox exports are re-exported from `@aletheics/zeno-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@aletheics/zeno-ai';
import { builtinModels } from '@aletheics/zeno-ai/providers/all';
...
import { openaiProvider } from '@aletheics/zeno-ai/providers/openai';
import { openrouterProvider } from '@aletheics/zeno-ai/providers/openrouter';
import { amazonBedrockProvider } from '@aletheics/zeno-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@aletheics/zeno-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@aletheics/zeno-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@aletheics/zeno-ai';
...
API-key credentials use the same discriminator as zeno's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@aletheics/zeno-ai';
...
```typescript
import { validateToolCall, type Tool } from '@aletheics/zeno-ai';
...
```typescript
import { builtinImagesModels } from '@aletheics/zeno-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@aletheics/zeno-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@aletheics/zeno-ai';
import { openAICompletionsApi } from '@aletheics/zeno-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@aletheics/zeno-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@aletheics/zeno-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are zeno thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@aletheics/zeno-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@aletheics/zeno-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@aletheics/zeno-ai';
import { anthropicProvider } from '@aletheics/zeno-ai/providers/anthropic';
import { openaiProvider } from '@aletheics/zeno-ai/providers/openai';
import { googleProvider } from '@aletheics/zeno-ai/providers/google';
...
- `@aletheics/zeno-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@aletheics/zeno-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@aletheics/zeno-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@aletheics/zeno-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@aletheics/zeno-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@aletheics/zeno-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @aletheics/zeno-ai login              # interactive provider selection
npx @aletheics/zeno-ai login anthropic    # login to specific provider
npx @aletheics/zeno-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@aletheics/zeno-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@aletheics/zeno-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@aletheics/zeno-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@aletheics/zeno-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@aletheics/zeno-ai`.
