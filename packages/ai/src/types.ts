import type { TelemetryContext } from "@aletheics/zeno-telemetry";
...
export interface StreamOptions extends ProviderRequestOptions<Model<Api>> {
	 ...
	 * SGLang, ...) receive parameters zeno does not model, e.g. `top_p`, `top_k`, `min_p`,
...
export interface OpenAICompletionsCompat {
	...
	/** Whether streamed responses include `finish_reason`. When false, zeno infers `stop` or `toolUse` when the stream ends. Default: true. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps zeno thinking levels to provider/model-specific values.
