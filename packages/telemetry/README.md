# @aletheics/zeno-telemetry

Vendor-neutral telemetry contracts and typed schema utilities for zeno packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Zeno packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Zeno Package Integration](#zeno-package-integration)
...
```bash
npm install @aletheics/zeno-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@aletheics/zeno-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@aletheics/zeno-telemetry';
...
`@aletheics/zeno-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@aletheics/zeno-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@aletheics/zeno-agent-core';
...
## Zeno Package Integration

Package ownership is intentionally split:

- `@aletheics/zeno-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@aletheics/zeno-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@aletheics/zeno-agent-core` owns and exports the zeno AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
import {
  ...
  startAiSpan,
  startHarnessSpan,
} from '@aletheics/zeno-agent-core';
```

The zeno schemas use pi-owned `pi.ai.*`, `pi.harness.*`, and `pi.session.*` names. Adapters may translate them to backend conventions without changing the emitted zeno vocabulary.
