...
const codingAgentName = "@aletheics/zeno-coding-agent";
const developmentPackages = new Set(
	["zeno-client", "zeno-protocol", "zeno-server"].map((name) => `@aletheics/${name}`),
);
...
for (const name of ["zeno-client", "zeno-protocol", "zeno-server"]) {
  assert.throws(() => import.meta.resolve("@aletheics/" + name), /Cannot find|cannot find/, name + " must not be installed");
}
...
`);
		...
		for (const cli of new Set([manifest.bin.zeno, "dist/cli.js"])) {
