...
const packages = [
	{ directory: "packages/chord", name: "@aletheics/chord" },
	{ directory: "packages/telemetry", name: "@aletheics/zeno-telemetry" },
	{ directory: "packages/ai", name: "@aletheics/zeno-ai" },
	{ directory: "packages/tui", name: "@aletheics/zeno-tui" },
	{ directory: "packages/agent", name: "@aletheics/zeno-agent-core" },
	{ directory: "packages/protocol", name: "@aletheics/zeno-protocol" },
	{ directory: "packages/client", name: "@aletheics/zeno-client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@aletheics/zeno-session-backend-sqlite-node" },
	{ directory: "packages/server", name: "@aletheics/zeno-server" },
	{ directory: "packages/coding-agent", name: "@aletheics/zeno-coding-agent" },
];
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `zeno-${platform}.zip` : `zeno-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "zeno.cmd"))) {
			writeFileSync(join(installDirectory, "zeno.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\zeno.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "zeno.ps1"), '& "$PSScriptRoot/node_modules/.bin/zeno.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "zeno.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\zeno.exe" %*\r\n');
		writeFileSync(join(installDirectory, "zeno.ps1"), '& "$PSScriptRoot/node_modules/.bin/zeno.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "zeno"), join(installDirectory, "zeno"));
}
...
if (rootPackageJson.name !== "zeno-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `zeno-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "zeno.exe" : "zeno")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "zeno.cmd" : "zeno")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "zeno.cmd" : "zeno")} --help`);
	}
}
