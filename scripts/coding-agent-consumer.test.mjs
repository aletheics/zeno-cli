...
const devPackages = ["zeno-client", "zeno-protocol", "zeno-server"].map((name) => `@aletheics/${name}`);
...
function createFixture(t, { importServer = false, declareServer = false } = {}) {
	...
	const packages = [codingAgentName, "@aletheics/chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				bin: { zeno: "dist/bundle/cli.js" },
				dependencies: {
					"@aletheics/chord": "1.0.0",
					...(declareServer ? { "@aletheics/zeno-server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@aletheics/zeno-server";' : ""}
import { marker } from "@aletheics/chord";
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@aletheics/zeno-server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@aletheics/zeno-server", version: "1.0.0" }));
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /zeno-server must not be installed/);
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@aletheics\/zeno-server'/);
});
