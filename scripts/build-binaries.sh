...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before zeno starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/zeno.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/zeno"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating zeno-$platform.zip..."
        (cd "$platform" && zip -r ../zeno-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating zeno-$platform.tar.gz..."
        mv "$platform" zeno && tar -czf zeno-$platform.tar.gz zeno && mv zeno "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../zeno-$platform.zip)
    else
        tar -xzf zeno-$platform.tar.gz && mv zeno "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/zeno.exe"
    else
        echo "  $OUTPUT_DIR/$platform/zeno"
    fi
done
