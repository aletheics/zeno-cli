...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		await writeManifest(root, "packages/ai", {
			name: "@aletheics/zeno-ai",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/coding-agent", {
			name: "@aletheics/zeno-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@aletheics/zeno-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@aletheics/zeno-coding-agent": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@aletheics/zeno-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@aletheics/zeno-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@aletheics/zeno-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@aletheics/zeno-coding-agent"], "^1.0.0");
